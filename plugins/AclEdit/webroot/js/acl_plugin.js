...
function init_register_user_controller_toggle_right(app_root_url, user_id, plugin, controller, missing_aco_text)
{
	...
	if (! (plugin == null || plugin == "" || typeof(plugin) == "undefined") )
		params['plugin'] = plugin;
	
	var url = app_root_url + "AclEdit/aros/admin_get_user_controller_permission/" + user_id;
				...
				success: function (data, textStatus) 
				{
					...
					for(var action in permissions)
					{
						...
						if(permissions[action] == true || permissions[action] == false)
						{
							...
							{
								icon_html = "<img src=\"" + app_root_url + "AclEdit/img/design/tick.png"  + "\" class=\"pointer\" alt=\"granted\" />";
								start_granted = true;
							}
							else
							{
								icon_html = "<img src=\"" + app_root_url + "AclEdit/img/design/cross.png"  + "\" class=\"pointer\" alt=\"denied\" />";
								start_granted = false;
							}
						...
						{
							icon_html = "<img src=\"" + app_root_url + "AclEdit/img/design/important16.png"  + "\" alt=\"" + missing_aco_text + "\" title=\"" + missing_aco_text + "\" />";
...
function register_user_toggle_right(start_granted, app_root_url, span_id, user_id, plugin, controller, action)
{
	...
	if(start_granted)
	{
		var url1 = app_root_url + "AclEdit/aros/admin_deny_user_permission/" + user_id;
		var url2 = app_root_url + "AclEdit/aros/admin_grant_user_permission/" + user_id;
	}
	else
	{
		var url1 = app_root_url + "AclEdit/aros/admin_grant_user_permission/" + user_id;
		var url2 = app_root_url + "AclEdit/aros/admin_deny_user_permission/" + user_id;
	}
...
function init_register_role_controller_toggle_right(app_root_url, role_id, plugin, controller, missing_aco_text)
{
	...
	var url = app_root_url + "AclEdit/aros/admin_get_role_controller_permission/" + role_id;
...
function register_role_toggle_right(start_granted, app_root_url, span_id, role_id, plugin, controller, action)
{
	...
	if(start_granted)
	{
		var url1 = app_root_url + "AclEdit/aros/admin_deny_role_permission/" + role_id;
		var url2 = app_root_url + "AclEdit/aros/admin_grant_role_permission/" + role_id;
	}
	else
	{
		var url1 = app_root_url + "AclEdit/aros/admin_grant_role_permission/" + role_id;
		var url2 = app_root_url + "AclEdit/aros/admin_deny_role_permission/" + role_id;
	}
