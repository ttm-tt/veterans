    ...
    <tr>
    	...
    	foreach($roles as $role)
    	{
    	    ...
    	    else
    	    {
    	        ...
    	        echo $this->Html->link($this->Html->image('/AclEdit/img/design/tick_disabled.png'), array('plugin' => 'AclEdit', 'controller' => 'aros', 'action' => 'update_user_role', 'user' => $user[$user_pk_name], 'role' => $role[$role_pk_name]), array('title' => $title, 'alt' => $title, 'escape' => false));
    	    }
    		...
    		foreach($ctrl_infos as $ctrl_info)
    		{
    	    	...
    	    	*/
    		    ...
    		    if(!in_array($controller_name . '_' . $user[$user_pk_name], $js_init_done))
    	        {
    	        	...
    	        	$this->Html->scriptBlock('init_register_user_controller_toggle_right("' . $this->Url->build('/', ['fullBase' => true]) . '", "' . $user[$user_pk_name] . '", "", "' . $controller_name . '", "' . __d('acl', 'The ACO node is probably missing. Please try to rebuild the ACOs first.') . '");', ['block' => true]);
    	        }
    	        ...
    	        foreach($plugin_methods as $method)
    	        {
    		    	...
    		    	*/
    	            ...
    	            if(!in_array($plugin_name . "_" . $plugin_ctrler_name . '_' . $user[$user_pk_name], $js_init_done))
    		        {
    		        	...
    		        	$this->Html->scriptBlock('init_register_user_controller_toggle_right("' . $this->Url->build('/', ['fullBase' => true]) . '", "' . $user[$user_pk_name] . '", "' . $plugin_name . '", "' . $plugin_ctrler_name . '", "' . __d('acl', 'The ACO node is probably missing. Please try to rebuild the ACOs first.') . '");', ['block' => true]);
    		        }
