...
class AclEditAppController extends AppController
{
	function initialize() : void {
		...
		$this->loadComponent('AclEdit.AclManager');
		$this->loadComponent('AclEdit.AclReflector');
	}
...
//	    	}
		...
		{
			$this->request->session()->setFlash(__d('acl', 'The role model name is unknown. The ACL plugin bootstrap.php file has to be loaded in order to work. (see the README file)'), 'flash_error', null, 'plugin_acl');
		}
	}
	
	function _check_files_updates()
	{
	    if($this->request->getParam('controller') != 'acos'
	        || ($this->request->getParam('action') != 'admin_synchronize' &&
	            $this->request->getParam('action') != 'admin_prune_acos' &&
	            $this->request->getParam('action') != 'admin_build_acl'))
	    {
	...
	{
	    $aco_path  = $this->request->getQuery('plugin') ?: '';
        $aco_path .= empty($aco_path) ? $this->request->getQuery('controller') : '/' . $this->request->getQuery('controller');
        $aco_path .= '/' . $this->request->getQuery('action');
        
        return $aco_path;
	}
	function _set_aco_variables()
	{
        $this->set('plugin', $this->request->getQuery('plugin') ?: '');
        $this->set('controller_name', $this->request->getQuery('controller'));
        $this->set('action', $this->request->getQuery('action'));
	}
