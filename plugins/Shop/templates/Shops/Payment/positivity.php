		...
		'success' : function(data) {
			if ($.isPlainObject(data)) {
				$.each(data, function(key, value) {
					$('form#positivity').append('<input type="hidden" name="' + key + '" value="' + value + '">');
				});
				$('form#positivity').submit();
			} else {
...
<div id="billing" class="order billing form">
	...
	<form id="positivity" method="post" accept-charset="UTF-8" action="<?=$submitUrl?>" >
		<?php 
