		...
		'success' : function(data) {
			if ($.isPlainObject(data)) {
				$.each(data, function(key, value) {
					$('form#sogecommerce').append('<input type="hidden" name="' + key + '" value="' + value + '">');
				});
				$('form#sogecommerce').submit();
			} else {
...
<div id="billing" class="order billing form">
	...
	<div class="hint">
	<?php
		echo __d('user', 'Payment will be processed in a secure way by {0}.', 'Societe General eCommerce') . '<br>';
		echo __d('user', 'After the payment is completed you will be redirected to the registration.') . '<br>';
	?>
	</div>
	
	<form id="sogecommerce" method="post" accept-charset="UTF-8" action="<?=$submitUrl?>" >
		<?php 
